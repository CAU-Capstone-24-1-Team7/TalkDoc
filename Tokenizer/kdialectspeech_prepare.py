...
def check_kdialectspeech_folders(base_dir, province_code):
    ...
    if province_code == 'total':
        ...
        talk_dir = os.path.join(base_dir, 'people/talk/' + province_code).replace('\\', '/')
        say_dir = os.path.join(base_dir, 'person/say/' + province_code).replace('\\', '/')
        st_dir = os.path.join(base_dir, 'person/st/' + province_code).replace('\\', '/')
    ...
    if not os.path.exists(say_dir):
        err_msg = (
            "the directory %s does not exist (it is expected in the "
            "kdialectspeech dataset)" % say_dir
        )
        raise OSError(err_msg)
...
def get_kdialectspeech_files(base_dir, province_code, file_ext='json'):
    ...
    if province_code == 'total':
        ...
        print (os.path.join(base_dir, 'people/talk/' + province_code) + file_ext)
        files = (
...
def create_csv(data_folder, save_folder, province_code, data_ratio, splited_wav_folder):
    ...
    """
    print("\nCSV\n")
    print(data_folder, province_code)
    print(get_kdialectspeech_files(data_folder, province_code, file_ext='json'))
